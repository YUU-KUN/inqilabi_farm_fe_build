          ...
          <div class="card p-3">
            <div class="table-responsive">
              <!-- <div class="text-center px-5" style="margin: 50px auto;">
                  ...
                  <br>
                  <div v-for="(pembayaran, index) in pembayarans" :key="pembayaran.id" class="row d-flex align-items-center mb-4">
                      <div class="col">
                        <h5 class="font-weight-normal">{{pembayaran.created_at | formatDate}}</h5>
                      </div>
                      <div class="col">
                        <h5 class="font-weight-normal">{{ pembayaran.nama}}</h5>
                      </div>
                      <div class="col">
                        <h5 v-if="pembayaran.package"><span class="font-weight-normal">{{pembayaran.package.nama}}</span> <span class="text-danger font-weight-bold"> {{pembayaran.package.variant}}</span></h5>
                      </div>
                      <div class="col">
                        <span v-if="pembayaran.package">
                          <h5 class="font-weight-normal" v-if="pembayaran.package.tipe == 'kambing_jantan'">Jantan</h5>
                          <h5 class="font-weight-normal" v-if="pembayaran.package.tipe == 'kambing_betina'">Betina</h5>
                          <h5 class="font-weight-normal" v-if="pembayaran.package.tipe == 'sapi'">Sapi</h5>
                        </span>
                      </div>
                      <div class="col">
                        <h5 class="font-weight-normal">{{pembayaran.jumlah}}</h5>
                      </div>
                      <div class="col">
                        <h5 class="font-weight-normal">{{pembayaran.total | rupiah}}</h5>
                      </div>
                      <div class="col">
                        <h5 class="font-weight-normal" v-if="pembayaran.proses == 'inqilabi_farm'">Inqilabi Farm</h5>
                        <h5 class="font-weight-normal" v-else>Dikirim</h5>
                      </div>
                      <div class="col">
                          <button v-if="pembayaran.bukti_transfer" data-toggle="modal" :data-target="'#modalBuktiTransfer'+index" class="btn btn-primary btn-sm font-weight-bold">Lihat</button>
                          <img v-if="pembayaran.bukti_transfer" :src="baseurl + pembayaran.bukti_transfer" alt="Bukti Pembayaran" class="img-thumbnail" width="50px">
                          <button v-else data-toggle="modal" :data-target="'#modalUploadBukti'+index" class="btn btn-primary btn-sm font-weight-bold">Upload Bukti Transfer</button>
                          <div class="modal fade" :id="'modalBuktiTransfer'+index" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered" role="document">
                              <div class="modal-content">
                                ...
                                <div class="modal-body">
                                  <img v-if="pembayaran.bukti_transfer" :src="baseurl + pembayaran.bukti_transfer" alt="Bukti Pembayaran" class="img-fluid" width="1000px" >
                                </div>
                      ...
                      <div class="col">
                        <button v-if="pembayaran.status == 'success'" class="btn btn-success btn-sm text-light font-weight-bold">Berhasil</button>
                        <button v-else-if="pembayaran.status == 'failed'" class="btn btn-danger btn-sm text-light font-weight-bold">Gagal</button>
